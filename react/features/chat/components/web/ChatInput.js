...
class ChatInput extends Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this._onAttachmentSendOpen = this._onAttachmentSendOpen.bind(this);
    }
        ...
        return (
            <div className = { `chat-input-container${this.state.message.trim().length ? ' populated' : ''}` }>
                <div id = 'chat-input' >
                    ...
                    <div className='send-button-container hide-button'>
                        <div
                            className='send-button'
                            onClick={this._onAttachmentSendOpen}
                            role='button'
                            tabIndex={0}>
                            <Icon src={IconShareDoc}/>
                        </div>
                    </div>
                </div>
            </div>
    ...
    _onAttachmentSendOpen: () => void;

    /** ......................................................
     * Open attachments sender plugin
     *
     * @private
     * @returns {void}
     */

    _onAttachmentSendOpen() {

        const StateData = APP.store.getState()['features/base/conference'];

        console.log('Attachment roomjid', StateData?.conference?.room?.roomjid);

        this._doToggleChat();

        const attachmentsSender = new window.AttachmentsSender(StateData?.conference?.room?.role, StateData?.conference?.room?.roomjid);

        attachmentsSender.mount('#attachmentsSender');
